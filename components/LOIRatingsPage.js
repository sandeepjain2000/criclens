...
export default function LOIRatingsPage() {
  ...
  useEffect(() => {
    fetch('/api/rankings-loi?action=basic&minInnings=10')
      .then(r => r.json())
      .then(data => {
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-start justify-between pb-2">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-black text-slate-900 m-0">ODI Ratings</h2>
            </div>
            <p className="text-slate-500 text-base m-0" style={{ marginLeft: '2.25rem' }}>
              Player performance rankings across Limited Overs Internationals
            </p>
          </div>
        </div>
        ...
        {loading ? (
          <div className="py-12 text-center text-slate-400 animate-pulse">
            <p className="font-bold">Loading ODI ratings...</p>
          </div>
        ) : !batsmen.length && !bowlers.length ? (
          <div className="bg-orange-50 border border-orange-200 rounded-2xl p-6">
            <p className="text-slate-600 text-sm m-0">
              <span className="font-bold">No data available yet.</span> Please run the T20/LOI downloader script on your machine to populate the database with ODI deliveries and player data. Once loaded, ratings will appear here.
            </p>
          </div>
        ) : (
              ...
              {[
                ...
                <button
                  ...
                  className={`flex-1 px-6 py-4 text-left transition-colors ${
                    activeTab === tab.id ? 'bg-orange-50 border-b-2 border-orange-600' : 'bg-white hover:bg-slate-50'
                  }`}
