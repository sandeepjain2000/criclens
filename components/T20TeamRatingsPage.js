...
import { Shield, Play, X, RefreshCw, Info, Zap, TrendingUp } from 'lucide-react';

/**
 * T20 Team Ratings Page
 * Displays team-level ratings based on average player impact scores
 */
export default function T20TeamRatingsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/team-rankings-t20?action=load')
      .then(r => r.json())
      .then(data => {
        if (data.teamRatings) {
  ...
  const handleCompute = () => {
    ...
    const url = '/api/team-rankings-t20?minInnings=10';
        ...
        <div className="flex items-start justify-between pb-2">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-black text-slate-900 m-0">T20 Team Ratings</h2>
            </div>
            <p className="text-slate-500 text-base m-0" style={{ marginLeft: '2.25rem' }}>
              Team strength based on average impact scores of top batsmen and bowlers.
            </p>
          </div>
          {computed && (
            <button
          ...
          <div className="px-6 pb-6 pt-6">
            <div className="flex gap-3 items-end">
              <button
                ...
                style={{ background: computing ? '#94a3b8' : '#0ea5e9', color: '#fff', border: 'none', cursor: computing ? 'not-allowed' : 'pointer' }}
              >
            ...
            {(computing || statusLog.length > 0) && (
              <div className="mt-6 bg-slate-50 border border-slate-200 rounded-2xl p-5">
                {computing && (
                  <div className="mb-3">
                    ...
                    <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                      <div className="h-2 rounded-full bg-cyan-500 transition-all duration-500" style={{ width: `${Math.min(100, progress)}%` }} />
                    </div>
                  </div>
                )}
                <div className="font-mono text-xs text-slate-600 space-y-0.5 max-h-36 overflow-y-auto custom-scrollbar" style={{ lineHeight: '1.6' }}>
                  {statusLog.map((msg, i) => (
                    <div key={i} className={msg.startsWith('✅') ? 'text-cyan-600 font-bold' : msg.startsWith('❌') ? 'text-red-500' : ''}>{msg}</div>
                  ))}
                </div>
              </div>
            )}
                      ...
                      {teamRatings.map((team) => (
                        <tr key={team.team_name} className="hover:bg-slate-50 transition-colors">
                          <td className="px-6 py-4 text-center">
                            <span className={`font-black text-sm ${team.impact_rank <= 5 ? 'text-cyan-600' : 'text-slate-500'}`}>
                              #{team.impact_rank}
                            </span>
                          </td>
                          ...
                          <td className="px-6 py-4 text-center font-mono font-bold text-cyan-700">{team.batting_avg_impact.toFixed(4)}</td>
                          <td className="px-6 py-4 text-center font-mono font-bold text-cyan-700">{team.bowling_avg_impact.toFixed(4)}</td>
                          <td className="px-6 py-4 text-center font-mono font-black text-base text-cyan-700 bg-cyan-50 rounded-lg">{team.combined_impact.toFixed(4)}</td>
